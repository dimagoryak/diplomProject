...
class CardModal extends Component {
  ...
  toggleRatePicker = () => {
    this.setState({ isRatePickerOpen: !this.state.isRatePickerOpen });
  };

  handleRequestClose = () => {
    const { isColorPickerOpen , isRatePickerOpen } = this.state;
    const { toggleCardEditor } = this.props;
    if (!isColorPickerOpen && !isRatePickerOpen) {
      toggleCardEditor();
    }
  };

  render() {
    const { newText, isColorPickerOpen, isRatePickerOpen, isTextareaFocused } = this.state;
    ...
    return (
      ...
      >
        ...
        >
          ...
          />
          {(card.date || checkboxes.total > 0 || card.rate!=0) && (
            <CardBadges date={card.date} checkboxes={checkboxes} rate={card.rate}/>
          )}
        </div>
        <CardOptions
          isColorPickerOpen={isColorPickerOpen}
          isRatePickerOpen={isRatePickerOpen}
          card={card}
          listId={listId}
          boundingRect={boundingRect}
          isCardNearRightBorder={isCardNearRightBorder}
          isThinDisplay={isThinDisplay}
          toggleRatePicker={this.toggleRatePicker}
